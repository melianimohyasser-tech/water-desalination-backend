// ================================================================
//  Water Desalination — Backend API v3
//  Node.js + Express + Turso (LibSQL)
//  ✅ إحصائيات، سجل أحداث، سرعة المضخات، مؤقت النظام
// ================================================================
...
app.post('/api/sensor', async (req, res) => {
  try {
    ...
    // ✅ تسجيل تغييرات مهمة
    if (prev.sys1 !== d.sys1)
      await logEvent('system', `النظام 1 ${d.sys1 ? 'بدأ' : 'توقف'}`, d.sys1 ? 'info' : 'warning');
    if (prev.sys3 !== d.sys3)
      await logEvent('system', `النظام 3 ${d.sys3 ? 'بدأ' : 'توقف'}`, d.sys3 ? 'info' : 'warning');
    if (prev.valve !== d.valve)
      await logEvent('valve', `الصمام ${d.valve ? 'فُتح' : 'أُغلق'}`, 'info');
    if (prev.mode !== d.mode)
      await logEvent('mode', `وضع التشغيل: ${d.mode ? 'يدوي' : 'تلقائي'}`, 'info');

    // ✅ تسجيل تنبيهات الحساسات
    if (d.ph > 0 && (d.ph < 6.5 || d.ph > 8.5))
      await logEvent('alert', `pH غير طبيعي: ${d.ph.toFixed(2)}`, 'danger');
    if (d.tds > 500)
      await logEvent('alert', `TDS مرتفع: ${d.tds.toFixed(0)} ppm`, 'warning');
...
// ================================================================
//  GET /api/sensor/history?limit=50&from=&to=
// ================================================================
...
// ================================================================
//  ✅ GET /api/stats?hours=24 — إحصائيات
// ================================================================
...
// ================================================================
//  ✅ GET /api/logs?limit=50&level= — سجل الأحداث
// ================================================================
...
app.post('/api/settings', async (req, res) => {
  try {
    ...
    // ✅ تحديث سرعات المضخات في الذاكرة
...
// ================================================================
//  ✅ GET /api/timer — مؤقت النظام
// ================================================================
app.get('/api/timer', (req, res) => {
  const now = new Date();
  const sys1Minutes = sys1StartTime
    ? Math.floor((now - sys1StartTime) / 60000) : null;
  const sys3Minutes = sys3StartTime
    ? Math.floor((now - sys3StartTime) / 60000) : null;

  res.json({
    sys1: { running: !!sys1StartTime, minutes: sys1Minutes },
    sys3: { running: !!sys3StartTime, minutes: sys3Minutes },
  });
});
...
app.post('/api/command', async (req, res) => {
  ...
  const valid = [
    ...
    'CMD:PUMP7_ON','CMD:PUMP7_OFF','CMD:START','CMD:STOP',
  ...
  await logEvent('command', `أمر مرسل: ${command}`, 'info');
...
app.get('/api/alerts', async (req, res) => {
  // جلب حدود التنبيه من قاعدة البيانات
